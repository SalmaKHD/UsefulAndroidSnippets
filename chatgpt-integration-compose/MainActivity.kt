...
import com.example.firstaiappcation.ui.theme.FirstAiAppcationTheme
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            FirstAiAppcationTheme {
                // A surface container using the 'background' color from the theme
                Surface(
                    modifier = Modifier.fillMaxSize(),
                    color = MaterialTheme.colorScheme.background,
                ) {
                    val viewModel = ChatGPTViewModel()
                    val uistate = viewModel._uistate
                    Text(text=uistate.gptResponse)
                    //  SummarizeRoute(viewModel) // explicitly for Gemeni
                }
            }
        }
    }
